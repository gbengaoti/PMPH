...
struct TempGlobsCuda {
    ...
    TempGlobsCuda(   const unsigned int& numX,
                    const unsigned int& numY,
                    const unsigned int& outer ) {
        unsigned int memUsageBytes = numX * numY * outer * sizeof(REAL);

        cudaMalloc((void**)&d_u, memUsageBytes);
        cudaMalloc((void**)&d_u_t, memUsageBytes);
        cudaMalloc((void**)&d_v, memUsageBytes);
        cudaMalloc((void**)&d_a, memUsageBytes);
        cudaMalloc((void**)&d_b, memUsageBytes);
        cudaMalloc((void**)&d_c, memUsageBytes);
        cudaMalloc((void**)&d_abc, 3*memUsageBytes);
        cudaMalloc((void**)&d_y, memUsageBytes);
        cudaMalloc((void**)&d_yy, memUsageBytes);
    }
