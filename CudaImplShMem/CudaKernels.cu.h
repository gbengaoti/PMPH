    ...
    unsigned int numX, unsigned int numY, unsigned int outer)  // array dimensions
{
    ...
    if (i < outer && j < numY && i1 < numX)
    {
        ...
        REAL x = d_x[i1];
        d_Result[loc] = max(x-(strikeCoefficient * i), (REAL)0.0);
    }
}
...
__global__ void  tridagShMemCuda(
          REAL*   d_a,   // size [n]
          REAL*   d_b,   // size [n]
          REAL*   d_c,   // size [n]
          REAL*   d_r,   // size [n]
    ...
    extern __shared__ REAL c[];

    const unsigned int tid = threadIdx.x;
    const unsigned int blockDimX = blockDim.x;
    const unsigned int j = blockIdx.x*blockDimX + tid;  // global id - m
    const unsigned int out = blockIdx.y*blockDim.y + threadIdx.y;   // global id - outer

    if (j < m && out < outer)
    {
        // copy c into shared memory
        // we have blockDimX threads, so the stride is blockDimX
        // there are n*blockDimX reals to copy, so we need to 
        // run n times
        unsigned int c_start = out*n*m + (j-tid)*n;
        for (unsigned int i = 0; i < n; ++i)
        {
            unsigned int loc_sh = i * blockDimX + tid;
            c[loc_sh] = d_c[c_start + loc_sh];
        }
        __syncthreads();
        ...
        unsigned int loc_c = threadIdx.x * n;
        for(int i=1; i<n; i++) {
            unsigned int loc_i = loc + i;
            
            REAL beta  = d_a[loc_i] / uu_prev;
            
            uu_prev = d_b[loc_i] - beta*c[loc_c + i-1];
            u_prev  = d_r[loc_i] - beta*u_prev;
        ...
        for(int i=n-2; i>=0; i--) {
            unsigned int loc_i = loc + i;
            u_prev = (d_u[loc_i] - c[loc_c + i] * u_prev) 
                            / d_uu[loc_i];
            d_u[loc_i] = u_prev;            
        }
    }
