...
/*__global__ void parInitOperator(Real* x, Real* Dxx, const unsigned n){
    // Using a one dimensional cuda kernel
    REAL dxl, dxu;
    int gid = blockIdx.x*blockDim.x + threadIdx.x; 
    if (gid < n){
    //  lower boundary - hard coding
    dxl      =  0.0;
    dxu      =  x[1] - x[0];
    ...
    { // Use CUDA here
        ...
        Dxx[gid][0] =  2.0/dxl/(dxl+dxu);
        Dxx[gid][1] = -2.0*(1.0/dxl + 1.0/dxu)/(dxl+dxu);
        Dxx[gid][2] =  2.0/dxu/(dxl+dxu);
        Dxx[gid][3] =  0.0; 
    }
    //  upper boundary
    dxl        =  x[n-1] - x[n-2];
