 ...
 */
template <class T, int TILE> 
__global__ void matTransposeTiledKer(T* A, T* B, int heightA, int widthA, int depth) {
  ...
  if( x < widthA && y < heightA && z < depth )
      shtileTR[threadIdx.y][threadIdx.x] = A[z*heightA*widthA + y*widthA + x];
  ...
  if( x < heightA && y < widthA && z < depth )
      B[z*heightA*widthA + y*heightA + x] = shtileTR[threadIdx.x][threadIdx.y];
}
...
void transpose( T*             inp_d,  
   ...
   dim3 block(tile, tile, 1);
   dim3 grid (dimx, dimy, depth);
 
   //2. execute the kernel
   matTransposeTiledKer<T,tile> <<< grid, block >>>
                       (inp_d, out_d, height, width, depth);    
   cudaThreadSynchronize();
}
